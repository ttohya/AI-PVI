# XL-Mix 5-Ensemble + TTA Evaluation
import os
...
test_imgs_list, test_masks_list = [], []
with open(csv_path, 'r') as f:
    reader = csv.DictReader(f)
    for row in reader:
        if float(row['AF_rec']) == 0.0 and int(row['khold']) == 4:
            img = np.array(Image.open(row['input_img']).resize((512, 512))) / 255.0
            mask = np.array(Image.open(row['mask1_img']).convert('L').resize((512, 512))) / 255.0
            test_imgs_list.append(img)
            test_masks_list.append(mask[:, :, np.newaxis])
...
for seed in [1, 2, 3, 4, 5]:
    ...
    model = build_model()
    path = f"/mnt/s/Workfolder/Ablation/PV_area/model/xl_mix_ens_{seed}.weights.h5"
    ...
    iou = calc_iou(y_true, np.argmax(pred, axis=-1), test_atrium)
    print(f"Model {seed}: IoU = {iou:.4f}")
    all_preds.append(pred)

    pred_tta = predict_tta(model, test_imgs)
    iou_tta = calc_iou(y_true, np.argmax(pred_tta, axis=-1), test_atrium)
